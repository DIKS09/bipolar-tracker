    ...
    if (filteredEntries.length === 0) {
        container.innerHTML = `
            <div class="empty-state">
                <p>📝 ${currentFilter === 'all' ? 'Пока нет записей. Добавьте первую запись выше!' : 'Нет записей для выбранного фильтра'}</p>
            </div>
        `;
        return;
    }
    ...
    container.innerHTML = filteredEntries.map(entry => {
        ...
        return `
            <div class="entry-item ${entry.mood}">
                ...
                <button class="entry-delete" onclick="deleteEntry(${entry.id})">🗑️ Удалить</button>
            </div>
        `;
    }).join('');
}
